...
在可变数组成为ISO C之前，其使用方式通过是如下方式实现的(以下代码来自[Nuttx](https://github.com/apache/incubator-nuttx))
...
相比于ISO C99的用法，这种用法，要求可变数组的长度至少为1。因此，在计算数组长度的时候，需要在减去1，以消除该成员带来的影响，使用方式上则与ISO C99上没有差异。倒不如说，正是由于C语言标准的发展滞后，才会出现这种写法，并最终发展为C语言标准的一部分。

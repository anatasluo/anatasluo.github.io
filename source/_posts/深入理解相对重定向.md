...
A: 因为流水线(pipeline)的存在，为了提高指令的指令效率，当前指令在执行时，EIP是执行下一条指令。
...
现在，引入以下变量和概念
instruction_len: 重定向指令的长度，上文的例子中为callq本身的1bytes和offset的4bytes，为5bytes
...
2. 相对偏移如果确定，则以section为基址进行重定向，可以减少符号表大小， Addend = - relocation_len
3. 相对偏移如果不确定，则以symbol为基址进行重定向，Addend = symbol_offset - relocation_len
...
## References
1. [https://www.felixcloutier.com/x86/call](call instruction描述)
2. [https://zhuanlan.zhihu.com/p/105936114](知乎专栏 - 静态链接与动态链接的宏观概述及微观详解)
3. [https://blog.cloudflare.com/how-to-execute-an-object-file-part-1/](blog - How to execute an object file)
4. [https://www.amd.com/system/files/TechDocs/24592.pdf](AMD SPEC)
